...
description: >
  ...
  Supports both PowerShell ISE and modern PowerShell environments with Standard User (non-admin) safety.
  
  CORE MISSION: 
  Ensure every command is executable, safe, object-oriented, and runs within Standard User privileges. 
  ...
  - Privilege Check: Never assume or require Local Administrator privileges.
  - Protected Paths: Never touch HKEY_LOCAL_MACHINE or system directories (e.g., C:\Windows\System32).
---
...
**Never** hard-code user paths (e.g., `C:\Users\Admin`). **Must** use environment variables or relative paths.
**Always** include `-Recurse` when using `Get-ChildItem` for file discovery.
...
$config = Join-Path -Path $env:USERPROFILE -ChildPath "config.json"

# Multiple path segments
$configFile = Join-Path -Path (Join-Path -Path $env:ProgramData -ChildPath "MyApp") -ChildPath "config.json"

# File discovery - always use -Recurse
Get-ChildItem -Path $searchRoot -Recurse -Filter "*.log"
```
...
All scripts must operate within **Standard User** constraints. Never assume, request, or provide commands requiring Local Administrator privileges.

- **Never** modify `HKEY_LOCAL_MACHINE` registry hives
- **Never** write to system directories (e.g., `C:\Windows\System32`)
- **Never** use `Set-ExecutionPolicy` at the Machine scope
- **Always** validate commands do not touch protected resources before outputting them

```powershell
# Never
Set-ItemProperty -Path "HKLM:\SOFTWARE\MyApp" -Name "Setting" -Value 1
Copy-Item -Path $file -Destination "C:\Windows\System32\"

# Must - use current-user scoped locations
Set-ItemProperty -Path "HKCU:\SOFTWARE\MyApp" -Name "Setting" -Value 1
Copy-Item -Path $file -Destination (Join-Path -Path $env:LOCALAPPDATA -ChildPath "MyApp")
```

### 8. Environment Isolation (Portable Execution)

For portable or ISE-based workflows, scripts should redirect environment variables to the current working directory. This ensures self-contained execution without depending on user-profile paths or pre-installed tools.

```powershell
# Portable environment initialization block
...
- Running in locked-down environments where user profile paths are restricted
- Building portable toolkits that bundle their own dependencies (JDK, Node, Python, Git)
- Operating in PowerShell ISE where external console interaction is limited

### 9. ISE Compatibility

When targeting PowerShell ISE, be aware of these constraints:

- `ForEach-Object -Parallel` is **not available** in ISE (PowerShell 5.1). Use `ForEach-Object` or `Start-Job` instead.
- Avoid commands that require external console interaction unless explicitly specified.
- Use ISE-friendly output methods (`Write-Output`, `Write-Warning`, `Write-Verbose`) over console-specific formatting.
...
Before providing any command, verify it does not touch protected registry hives (e.g., `HKEY_LOCAL_MACHINE`) or system directories (e.g., `C:\Windows\System32`).
...
- [ ] **Did I hard-code any user paths?** Never use `C:\Users\...` — use `$env:USERPROFILE`, `$env:APPDATA`, etc.
...
- [ ] **Am I staying within Standard User privileges?** No `HKLM:` writes, no system directory modifications, no admin-only cmdlets.
- [ ] **Did I include -Recurse for file discovery?** `Get-ChildItem` calls for discovery should use `-Recurse`.
- [ ] **Is this ISE-compatible?** If targeting ISE, no `ForEach-Object -Parallel` or external console dependencies.
...
### Secure File Download (.NET Fallback)

In restricted environments where `Invoke-WebRequest` or `Invoke-RestMethod` may be unavailable or blocked by execution policy, use this .NET-based download method:
...
Use this when:
- Running in PowerShell ISE where `Invoke-WebRequest` may not be available
- Execution policies block cmdlet-based downloads
- You need a dependency-free download mechanism

## Acknowledgments

- Original `agent-powershell-standardizer` skill by [young (hqy2435662352)](https://github.com/hqy2435662352/agent-powershell-standardizer) — MIT License
- ISE Specialist and portable execution patterns contributed by [freeload101](https://github.com/freeload101)
